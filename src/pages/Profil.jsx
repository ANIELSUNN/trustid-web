...
export default function Profil() {
  ...
  async function chercher() {
    ...
    setChargement(true); setErreur(''); setProfil(null);
  ...
  return (
    <div style={s.page}>
      ...
      <p style={s.sous}>Recherchez un utilisateur TrustID par email ou identifiant</p>
        ...
        <div style={s.carte}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginBottom: 24 }}>
            ...
            <div>
              ...
              <span style={s.badge}>● {profil.certificat?.statut?.toUpperCase() || 'VÉRIFIÉ'}</span>
            </div>
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
            <div style={s.champ}>
              ...
              <p style={s.val}>{profil.certificat?.emis ? new Date(profil.certificat.emis).toLocaleDateString('fr-FR') : '—'}</p>
            </div>
            <div style={s.champ}>
              <p style={s.label}>Expire le</p>
              <p style={s.val}>{profil.certificat?.expire ? new Date(profil.certificat.expire).toLocaleDateString('fr-FR') : '—'}</p>
            </div>
          </div>

          <div style={s.qr}>
            <QRCodeSVG value={`http://localhost:3000/profil?id=${profil.id || profil._id}`} size={100} />
